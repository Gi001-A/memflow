...
class MemFlowNet(nn.Module):
    def __init__(self, cfg):
        ...
        if cfg.cnet == 'twins':                                     #这里用的module都要再看看
    ...
    def encode_context(self, frame):
        ...
        net, inp = torch.split(cnet, [self.hidden_dim, self.context_dim], dim=1)   #net用于表示网络内部状态，inp表示网络输入
        net = torch.tanh(net)
        inp = torch.relu(inp)
        query, key = self.att.to_qk(inp).chunk(2, dim=1)   #inp生成q与k
    ...
    def predict_flow(self, net, inp, coords0, coords1, fmaps, query, ref_keys, ref_values, test_mode=False):
        ...
        for _ in range(self.cfg.decoder_depth):
            ...
            corr = corr_fn(coords1)  # index correlation volume  获得相关性
            flow = coords1 - coords0

            #这里稍微不太对   如果用gma模块，则没有这个函数
            motion_features, current_value = self.update_block.get_motion_and_value(flow, corr)
            current_value = current_value.unsqueeze(2)
            value = current_value if ref_values is None else torch.cat([ref_values, current_value], dim=2)  #更新value buffer
            ...
            flow_predictions.append(flow_up)      #这个的目的是获取不同分辨率的流预测吗？
