...
class BasicMotionEncoder(nn.Module):   #得到运动特征
    ...
    def forward(self, flow, corr):    #分别为   flow：当前的流预测    corr：两帧间的相关性
...
class BasicUpdateBlock(nn.Module):
    ...
    def forward(self, net, inp, corr, flow, upsample=True):
        ...
        inp = torch.cat([inp, motion_features], dim=1)    #这行什么意思？
...
class GMAUpdateBlock(nn.Module):    #拼接3个特征，进行流预测
    ...
    def forward(self, net, inp, corr, flow, attention):
        ...
        delta_flow = self.flow_head(net)    #用gru单元处理特征信息，进行流预测
